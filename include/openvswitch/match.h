...
void match_set_tun_gtpu_msgtype_masked(struct match *match, uint8_t msgtype,
                                       uint8_t mask);
void match_set_tun_eth_src_masked(struct match *match, 
                                   const struct eth_addr dl_src,
                                  const struct eth_addr mask);
void match_set_tun_eth_src(struct match *match, const struct eth_addr dl_src);
void match_set_tun_eth_dst_masked(struct match *match, 
                                   const struct eth_addr dl_dst,
                                  const struct eth_addr mask);
void match_set_tun_eth_dst(struct match *match, const struct eth_addr dl_dst);
void match_set_tun_vlan_id(struct match *match, ovs_be16 vid);
void match_set_tun_vlan_id_masked(struct match *match,
                                   ovs_be16 vid, ovs_be16 mask);
void match_set_tun_dl_port(struct match *match, ovs_be32 odp_port);
void match_set_tun_dl_port_masked(struct match *match,
